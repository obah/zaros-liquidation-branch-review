...
The Position library is being initialized and attached to the Data struct from  Position library. This makes functions like getState(),  getAccruedFunding(), update() which are from Position library available to the Data struct when it’s used in the  LiquidationBranch smart contract.
 tOrder library is being initialized and attached to the Data struct from the MarketOrder library. This makes functions like checkPendingOrder(),  update(), clear() which are from the MarketOrder library available to the Data struct when it’s used in the LiquidationBranch smart contract.
